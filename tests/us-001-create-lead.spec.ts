...
test.describe('US-001 — Create Lead E2E', () => {
  ...
  test('E2E: Validation error shown when Company is left blank during Lead creation', { tag: ["@e2e","@regression","@lead","@P0","@case-1d359d6b-c158-4800-885b-1fd23364f0fb","@req-0f79ec14-b4b4-41c4-9fcc-3a26fb6f1292"] }, async ({ page, homePage, leadListPage, commonPage }) => {
  await test.step('Click — Navigate to Leads tab', async () => {
    await homePage.clickLeads();
  });
  await test.step('Click — Click New button', async () => {
  ...
  await test.step('Fill — Enter First Name', async () => {
    await leadListPage.fillFirstName('John');
  });
  await test.step('Fill — Enter Last Name only, leave Company blank', async () => {
    await leadListPage.fillLastName('Smith');
  });
  await test.step('Click — Click Save button', async () => {
    await leadListPage.clickSaveEdit();
  });
  await test.step('Assert visible — Validation error appears for Company field — (text=/Complete this field|required/i)', async () => {
    await commonPage.expectValidationErrorAppearsForCompanyFieldVisible();
  });
  await test.step('Assert visible — User remains on New Lead form (not saved)', async () => {
    await leadListPage.expectNewLeadVisible();
  });
